...
def read_data_sets(len_back, len_front, test_size=1392):
...
def produce_data2(len_back, len_front, order):
    local_file = 'trajactory.txt'
    with open(local_file, 'rb') as f:
        raw_data = []
        for line in f:
...
def produce_data3(len_back, len_front, order):
    local_file = 'trajactory.txt'
    with open(local_file, 'rb') as f:
        raw_data = []
        for line in f:
...
def produce_data4(len_back, len_front, order):
    local_file = 'trajactory.txt'
    with open(local_file, 'rb') as f:
        raw_data = []
        for line in f:
    ...
    for idx in range(len_back, (raw_data.shape[0]-len_front)):
        ...
        accum = 0
        delta = np.zeros(2*len_front)
        laststepx = data[(len_back-1), 1]
        laststepy = data[(len_back-1), 2]
        for i in range(0, len_front):
            accum += 0.1
            delta[i * 2] = params[0] + accum * params[1] + accum**2 * params[2] - data[(len_back-1), 1]
            delta[i*2+1] = params[3] + accum * params[4] + accum**2 * params[5] - data[(len_back-1), 2]
            thisstepx = params[0] + accum * params[1] + accum ** 2 * params[2]
            thisstepy = params[3] + accum * params[4] + accum ** 2 * params[5]
            delta[i * 2] = thisstepx - laststepx
            delta[i*2+1] = thisstepy - laststepy
            laststepx = thisstepx
            laststepy = thisstepy
        train_label.append(np.reshape(delta, -1))
